...
class WeightedLoss(tf.keras.losses.Loss):
  def __init__(self, base_loss, attention_ids, weight, batch_size=32, custom_metric_dict=None, reduction=None):
  ...
  def call(self, y_true, y_pred):
    ...
    if self.custom_metric:
       custom_loss = self.custom_metric.__call__(y_true * self.weights, y_pred * self.weights)
    ...
    return weighted_loss + custom_loss
  # * 2 ** self.weight
  ...
  if __name__ == "__main__":
    ...
    loss = WeightedLoss(tf.keras.losses.MeanSquaredError(), np.load("./weight_indices.npy"), 1, 1)
    print("In")
