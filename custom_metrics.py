...
class custom_metric():
    ...
    def __call__(self, y_test, y_pred):
        ...
        loss = self.custom_hausdorff(y_test, y_pred)
    ...
    def custom_hausdorff(self, pred_coords, true_coords):
        squared_distances = tf.reduce_sum(tf.square(pred_coords - true_coords), axis=-1)

        # Find the maximum distance for each point across the second dimension (num_points)
        max_distances = tf.reduce_max(squared_distances, axis=1)

        # Take the square root to obtain the actual distances
        max_distances = tf.sqrt(max_distances)
